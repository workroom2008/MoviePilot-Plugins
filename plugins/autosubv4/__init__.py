...
from plugins.AutoSubv4.ffmpeg import Ffmpeg
from plugins.AutoSubv4.translate.openai_translate import OpenAi
...
class AutoSubv4(_PluginBase):
    # 插件名称
    plugin_name = "AI字幕自动生成(法文版)"
    # 插件描述
    plugin_desc = "专门用于法文视频的字幕自动生成和翻译"
    # 插件图标
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "AutoSubv4"
    # 加载顺序
    plugin_order = 14
    # 可使用的用户级别
    auth_level = 2
    ...
    def init_plugin(self, config=None):
        ...
        # 字幕生成设置
        self._translate_preference = config.get('translate_preference', 'french_first')
    ...
    def __do_speech_recognition(self, audio_lang, audio_file):
        ...
        try:
            ...
            segments, info = model.transcribe(audio_file,
                                              language=lang if lang != 'auto' else 'fr',  # 确保使用法文
    ...
    def __generate_subtitle(self, video_file, subtitle_file, enable_asr=True):
        ...
        # 获取字幕语言偏好
        if self._translate_preference == "french_only":
            prefer_subtitle_langs = ['fr', 'fre', 'fra']
            strict = True
        elif self._translate_preference == "french_first":
            prefer_subtitle_langs = ['fr', 'fre', 'fra']
            strict = False
        ...
        if audio_lang and audio_lang != 'und':
            # 如果音轨语言是英文或其他语言，强制改为法文
            if audio_lang in ['en', 'eng', 'ja', 'jpn']:
                logger.info(f"检测到非法语音轨，强制使用法文识别")
                audio_lang = 'fr'
            elif not iso639.find(audio_lang) or not iso639.to_iso639_1(audio_lang):
                logger.info(f"字幕源偏好：{self._translate_preference} 未从音轨元数据中获取到有效语言信息，使用法文")
                audio_lang = 'fr'
        else:
            logger.info(f"字幕源偏好：{self._translate_preference} 未从音轨元数据中获取到语言信息，使用法文")
            audio_lang = 'fr'
            
        # 当字幕源偏好为origin_first时，优先使用音轨语言
        if self._translate_preference == "origin_first":
            prefer_subtitle_langs = ['fr', 'fre', 'fra'] if audio_lang == 'auto' else [audio_lang,
        ...
        else:  # french_first/origin_first
    ...
    def __translate_zh_subtitle(self, source_lang: str, source_subtitle: str, dest_subtitle: str):
        ...
        if source_lang in ["fr", "fre", "fra"] and self._enable_merge:
            valid_subs = self.__merge_srt(subs)
            logger.info(f"法文字幕合并：合并前字幕数: {len(subs)},合并后字幕数: {len(valid_subs)}")
        else:
            valid_subs = subs
    ...
    def __target_subtitle_exists(self, video_file):
        ...
        if self._translate_zh:
            ...
            if self._translate_preference == "french_first":
                prefer_langs = ['fr', 'fre', 'fra']
                strict = False
            elif self._translate_preference == "french_only":
                prefer_langs = ['fr', 'fre', 'fra']
                strict = True
            else:
                prefer_langs = None
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        return [
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            ...
                                            'items': [
                                                {'title': '仅法文', 'value': 'french_only'},
                                                {'title': '法文优先', 'value': 'french_first'},
                                ...
                                'content': [
                                    {
                                        ...
                                        'content': [
                                            ...
                                            },
                                            {
                                                'component': 'a',
                                                'props': {
                                                    'href': 'https://github.com/TimoYoung/MoviePilot-Plugins/blob/main/plugins/AutoSubv4/README.md',
                                                    'target': '_blank'
                                                },
                                                'content': [
                                                    {
            ...
            "path_list": "",
            "file_size": "10",
            "translate_preference": "french_first",
            "translate_zh": False,
            "enable_asr": True,
